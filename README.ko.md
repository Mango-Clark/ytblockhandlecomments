...
기본 흐름은 여전히 handle 차단이며, `v0.4.0-pre1`에서는 YouTube Data API v3
`channels.list(forHandle)`를 사용하는 선택적 UID 기반 감지와 handle↔UID pair 관리가
추가되었습니다.
...
7. UID 감지 토글, pair 생성/갱신, regex 추가, import/export를 사용합니다.
...
6. 이후 stale 또는 mismatch가 보이면 `Update Pair`를 실행합니다.
...
- `v0.4.0-pre1`에서는 pair 메타데이터를 import/export에 포함하지 않습니다
