...
기본 흐름은 여전히 handle 차단이며, `v0.5.0`은 `v0.4.x`의 관리자와 Shorts 지원을
유지하면서 긴 세션 메모리 정리, 불필요한 pair update 조회 감소, 관리자 대화상자의
스크립트 버전 표시를 추가합니다.
...
- `v0.5.0`에서도 pair 메타데이터를 import/export에 포함하지 않습니다
