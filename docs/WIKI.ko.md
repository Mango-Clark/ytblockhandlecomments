...
- console 로그 기본 prefix는 `[YTCB]`, 시간 표시는 꺼짐입니다. preset은 확장·basic calendar date, week date, ordinal date, time을 지원합니다. 직접 ISO 형식은 `yyyy`, `yy`, `MM`, `dd`, `DDD`, `ww`, `e`, `HH`, `mm`, `ss`, `SSS`, `X`, `XXX`, `Z`, `T`, `W`를 조합하며 basic/extended time에 timezone token을 함께 쓸 수 있습니다. timezone은 system, `-12`~`+14` UTC offset, 목록 IANA 도시, 검증한 직접 IANA 또는 KST식 약어를 지원합니다.
...
- debug metrics는 page mode, comments host·extraction failure counter, 최근 selector reason을 표시하며 댓글 본문, URL, API 키, 계정 정보는 포함하지 않습니다
...
`v1.5.0` 이후에는 큰 관리자/보안/i18n/regex-selection 성능/Shorts 댓글 숨김/긴 세션
