...
- 댓글 작업 분할·검색 지연·자동 페어 조회 중단·수동 페어 순차 처리를 적용하는 선택형 저성능 모드 추가.
- 차단 목록·페어 결과를 일반 모드 100개, 저성능 모드 50개씩 표시하고 페이지 간 선택을 보존.
- 저장 항목 상태·콘솔 미리보기·테스트 출력·빈 상태 동작·확인 후 삭제를 포함한 그룹별 로그 설정 추가.

### Changed

- 차단 목록 controller가 선택·필터·페이지·cache·예약 검색 작업과 다이얼로그 정리를 소유하도록 개선하고, API 테스트·페어링 진행 상태를 별도 lifecycle 소유자로 분리하여 닫기·재오픈 후 이전 결과를 무시.
- userscript와 문서 생성물을 교체하기 전에 완전히 작성하여 부분 생성물 노출을 막도록 개선.
...
- 고정 반복 횟수라도 대안식 반복과 중첩 가변 반복을 거부하여 허용된 regex가 많은 handle을 검사할 때 관리자 화면을 멈추는 문제를 수정.

- 중첩 그룹·연속 반복을 이용한 regex safety 검사 우회로 브라우저가 멈추는 문제를 수정. 저장·가져오기 규칙과 탭 간 갱신을 포함해 매칭 전 보수적 복잡도 검사를 적용하며 지원하지 않는 표현식은 건너뜀.

- 민감한 진단 필드를 생략하고 JWT와 API 키 형태 문자열을 포함해 식별 가능한 자격 증명·식별자 값을 제거.
