...
- 로그 payload용 V0-V5 진단 상세도 설정을 추가했습니다.
- 차단 목록 내보내기 dialog에 JSON/텍스트 파일 다운로드와 목록으로 돌아가기 동작을 추가했습니다.
- 키워드 자동 처리 설정의 각 선택사항 설명을 추가했습니다.
- 정규식 규칙과 세부 키워드 설정을 위한 차단 및 키워드 자동 처리 통합 창을 추가했습니다.
- 기기설정, yt설정, 반전, 커스텀을 포함한 userscript UI 테마 선택 기능을 추가했습니다.

### Changed

- 설정 매칭 그룹 위 간격을 추가하고, 설정 초기화 동작을 파괴적 작업으로 강조했습니다.
- 설정에는 키워드 자동 처리 전체 켜기/끄기만 남기고 세부 설정을 통합 창으로 옮겼습니다.
...
- 재사용하거나 늦게 생성되는 YouTube popup에서도 댓글 `⋯` 메뉴 항목을 중복 없이 다시 주입합니다.
...
- 로컬 로그 보관 및 브라우저 console 로그를 독립적으로 설정하고, 로그 수준/보관 수 설정과 텍스트 파일 다운로드를 추가했습니다.
...
- 댓글 본문, 작성자 handle, 고정 표시 문구용 키워드 자동 처리를 추가했습니다.

### Changed

- pair 최초 생성은 갱신 검사 설정과 관계없이 계속 handle에서 UID를 조회합니다.
- 키워드 동작은 댓글별로 중복 실행되지 않고, 차단 목록 및 pair 요청도 중복되지 않게 처리합니다.
...
- 설정 dialog를 그룹화된 컨트롤, 설명, 자동 저장 안내를 포함한 카테고리 목록 레이아웃으로 변경했습니다.
...
- API 키 테스트와 pair 생성/갱신이 API 응답을 기다리는 동안 loading bar를 표시합니다.
- 설정 dialog와 차단 목록 사이를 오가는 버튼을 추가했습니다.
- 글자 크기와 UI 크기를 각각 5단계로 조절하는 설정을 추가했습니다.
...
- 차단 목록 dialog에 기존 설정, API, pair 섹션이 실제 본문에 붙도록 수정했습니다.
