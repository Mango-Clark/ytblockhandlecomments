...
- 표시·테마·pair 정책·API 설정 UI 변경은 matcher를 재구성하거나 댓글을 재검사하지 않고 dialog만 갱신하도록 제한.
...
- 규칙을 제거하면 화면 밖에서 숨겨진 댓글도 복원하고 관찰이 끝난 댓글에 늦게 도착한 intersection callback을 무시하도록 수정.
